...
class Excel_Stats:
    ...
    def __init__(self, sample_name):
        ...
        self.excel_filename = os.path.join(
            args.outdir, f"{sample_name}_{date_stamp}_stats.xlsx"
        )
        excel_dict = {}
...
class Blast_Fasta(bcolors):
    ...
    def __init__(
        ...
        blastout_file_path = os.path.join(args.outdir, blastout_file)
        subprocess.run(
            [
                "blastn",
                "-query",
        ...
        self.blastout_file = blastout_file_path

        blast_dict = defaultdict(list)
        with open(blastout_file_path, "r") as blast_file:
        ...
        descriptions = {}
        with open(
            os.path.join(args.outdir, f"{sample_name}_blast_all.txt"), "w"
        ) as all_blast:
        ...
        sorted_norm_dict = {
            k: v for k, v in sorted(norm_dict.items(), key=lambda item: item[1])
        }
        ...
        with open(
            os.path.join(args.outdir, f"{sample_name}_blast_summary.txt"), "w"
        ) as summary_blast:
            for acc, count in sorted_norm_dict.items():
                ...
                print(
                    f"{count:,}\t{acc_count[acc]:,}\t{acc} {descriptions[acc]}",
                    file=summary_blast,
                )
        ...
        try:
            highest_hit_accession = max(
                sorted_norm_dict.items(), key=operator.itemgetter(1)
            )[0]
...
class GenoFLU:
    ...
    def __init__(
        ...
        if sample_name:
            ...
            ## is this what breaks underscores in filenames?
            ## yes, because it matches from underscore to the end of the line
            ## So EPI_ISL_0000 becomes "EPI"
            sample_name = re.sub("[_.].*", "", FASTA_name)
        self.sample_name = sample_name
        ...
        if FASTA_dir:
            ...
            self.FASTA_dir = os.path.abspath(os.path.join(f"{script_path}", "..", "dependencies", "fastas"))
            self.cross_reference = os.path.abspath(
                os.path.join(f"{script_path}", "..", "dependencies", "genotype_key.xlsx")
            )
    ...
    def blast_hpai_genomes(
        ...
        with open(blast_hpai_genotyping.blastout_file, "r") as blastout:
            for line in blastout:
                ...
                if fasta_name != ind_blast_result[0]:
                    ...
                    blast_genotyping_hpia[ind_blast_result[-1].split()[2]] = (
                        each_blast  # just the gene as key
                    )
        ...
        for index, row in df.iterrows():
            dictionary_of_genotypes[row["Genotype"]] = {
                "PB2": row["PB2"],
                "PB1": row["PB1"],
                "PA": row["PA"],
                "HA": row["HA"],
...
if __name__ == "__main__":  # execute if directly access by the interpreter
    ...
    -o is an output directory to save all the reports into. Defaults to the current working directory
    ...
    parser.add_argument(
        ...
        action="store",
        dest="outdir",
        help="Output directory for results.  Default current working directory",
        default=os.getcwd(),
    )
    ...
    # fail hopelessly if it doesn't
    if not os.path.isdir(args.outdir):
        ## maybe it would be nice if it would make it for you
        ## instead refusing only if it already exists and isn't empty...
        raise NotADirectoryError(f"Output directory {args.outdir} does not exist")
    ...
    excel_stats.excel_dict["Genotype Average Depth of Coverage List"] = (
